...
Spotify는 필수 설정이 아닙니다. 리서치 중 Spotify 카탈로그 정보가 실제로 도움이
되는데 연결되어 있지 않다면, 에이전트가 이유를 설명하고 연결 여부를 물은 뒤
macOS 보안 입력창을 열 수 있습니다. Client Secret을 대화창에 붙여 넣을 필요가
없습니다.

Spotify 앱을 미리 연결하고 싶다면 별도 라이브러리 설치 없이 저장소 폴더에서
한 번만 아래 명령을 실행하면 됩니다.

```bash
python3 skills/dig-music/scripts/spotify_api.py configure
```
...
에이전트가 대화 중 설정을 진행할 때 사용하는 네이티브 창 명령은 다음과
같습니다.

```bash
python3 skills/dig-music/scripts/spotify_api.py configure --gui
```

```bash
python3 skills/dig-music/scripts/spotify_api.py check
python3 skills/dig-music/scripts/spotify_api.py search artist "CORTIS" --market KR
python3 skills/dig-music/scripts/spotify_api.py search album "CORTIS" --market KR
```
...
환경변수 방식은 [Spotify 공급자 문서](./skills/dig-music/providers/spotify.md)를
참고하세요.
